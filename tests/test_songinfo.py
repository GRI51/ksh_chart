...
class TestSongInfo:
    ...
    @pytest.mark.parametrize('ksh_path', [
        ...
        (os.path.join(os.path.dirname(
            __file__), 'testksh', 'boueinohakobune_Six[connect]', 'boueinohakobune[connect].ksh')),
    ])
    def test_search_ksh_element(self, ksh_path) -> None:
    ...
    @pytest.mark.parametrize('ksh_path, song_info', [
        (os.path.join(os.path.dirname(
            __file__), 'testksh', 'AstralSpirits[PIN]', 'AstralSpirits_ex[PIN].ksh'),
            {'title': 'Astral spirits[PIN]', 'artist': 'K-forest', 'effect': 'GRI', 'EX': '17', 'source': 'SFES2019', 'information': None}),
        (os.path.join(os.path.dirname(
            __file__), 'testksh', 'DirigeantDecision[respectN]', 'IN[respectN].ksh'),
            {'title': 'Dirigeant Decision[respectF]', 'artist': 'siqlo', 'effect': 'GRI', 'IN': '17', 'source': 'SF2016 No.31-60', 'information': None}),
        (os.path.join(os.path.dirname(
            __file__), 'testksh', 'Rv-27 [modern]', 'Rv-27 [modern].ksh'),
            {'title': '%UnDeciphered-CryptoGraph in the Edifice%[modern]', 'artist': 'seatrus', 'effect': 'GRI', 'IN': '18', 'source': 'Eupholic Selections vol.1', 'information': 'Genre:Revolution Hardcore'}),
        (os.path.join(os.path.dirname(
            __file__), 'testksh', 'イザヨイレイバース[PAN]', 'izayoiravers[PAN].ksh'),
            {'title': 'イザヨイレイバース[PAN]', 'artist': 'brz1128', 'effect': 'GRI', 'IN': '17', 'source': 'Pastel breeze vol.3', 'information': None}),
        (os.path.join(os.path.dirname(
            __file__), 'testksh', 'boueinohakobune_Six[connect]', 'boueinohakobune[connect].ksh'),
            {'title': '望影の方舟Six[connect]', 'artist': 'Se-U-Ra', 'effect': 'GRI', 'IN': '20', 'source': '天界の使者～Battle of angels～', 'information': '20.3'}),
    ])
    def test_songinfo(self, ksh_path: str, song_info: dict) -> None:
        ...
        try:
            expect_in = song_info.get('information')
            expect_in = str(float(expect_in))
        except (TypeError, ValueError):
            expect_in = song_info.get('IN')
        assert s_info.get('IN') == expect_in, 'INの難易度（1～20）が一致していません'
        assert s_info['source'] == song_info['source'], '出典が一致していません'
        assert s_info['information'] == song_info['information'], '情報が一致していません'

    def test_songinfo_filenotfound(self) -> None:
    ...
    def test_get_package_song_info_type_error(self, ksh_path, expect) -> None:
    ...
    def test_get_package_song_info_filenotfound_error(self, ksh_path, expect) -> None:
        with pytest.raises(FileNotFoundError) as err:
            songinfo.get_package_song_info(ksh_path)
        # エラーメッセージを検証
        assert str(err.value) == expect, 'エラーメッセージが期待値と一致しません。'
    ...
    def test_get_package_song_info_value_error(self, ksh_path, expect) -> None:
